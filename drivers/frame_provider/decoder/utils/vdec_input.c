		...
		size_t count, u32 handle, chunk_free free, void* priv)
{
	...
	if (input->total_wr_count + (count + need_padding_size) < 0) {
		input->total_wr_count -= input->total_rd_count;
		input->total_rd_count = 0;
	}
	input->total_wr_count += count + need_padding_size;
	vdec_input_unlock(input, flags);
#if 0
	if (add_count == 2)
			...
			struct vframe_chunk_s *chunk)
{
	...
	if (chunk->pts_valid) {
		...
		input->last_consumed_no_pts_cnt++;
	block->rp += chunk->size + chunk->padding_size;
	if (block->rp >= block->size)
		block->rp -= block->size;
	block->data_size -= chunk->size;
	block->chunk_count--;
	input->data_size -= chunk->size;
	input->total_rd_count += chunk->size + chunk->padding_size;
	if (block->is_out_buf) {
		list_move_tail(&block->list,
			&input->vframe_block_free_list);
		if (block->free) {
