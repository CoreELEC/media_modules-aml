...
static void set_cuva_data(struct AVS2Decoder_s *dec)
{
	...
	avs2_print(dec, AVS2_DBG_BUFMGR_MORE,
			"%s:pic %p cuva_count(%d) cuva_size(%d) hdr_flag 0x%x\n",
					...
					struct vdec_comp_buf_info *info)
{
	...
	info->frame_buffer_size = avs2_mmu_page_num(
		dec, dec->frame_width, height, bit_depth == AVS2_BITS_10);
...
static int file_write(struct file* fp, char *wdata, unsigned int size)
{
	...
	set_fs(KERNEL_DS);
	while (ret < size) {
		ret += vfs_write(fp, wdata + ret, size - ret, &fp->f_pos);
		if (++cnt > 2)
			break;
	}
	set_fs(old_fs);
				...
				if (pic->bg_flag == 0 &&
					...
					if (pic->refered_by_others == 0) {
						struct internal_comp_buf *ibuf =
							index_to_icomp_buf(dec, pic->index);
						int ret = 99;
#ifdef AVS2_10B_MMU
...
static void  avs2_decode_ctx_reset(struct AVS2Decoder_s *dec)
{
	...
	for (i = 0; i < REF_MAXBUFFER; ++i) {
		...
		avs2_dec->frm_pool[i].backend_ref = 0;
		...
		avs2_dec->frm_pool[i].back_done_mark = 0;
	}
