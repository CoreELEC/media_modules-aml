...
void BackEnd_StartDecoding(struct AV1HW_s *hw)
{
	...
	if (hw->front_back_mode == 1) {
			...
			ret = av1_alloc_mmu(hw, ibuf->mmu_box_1,
				pic->index,
				pic->y_crop_width,
				pic->y_crop_height/2 + 64 + 8,
				hw->aom_param.p.bit_depth,
...
#ifdef AOM_AV1_MMU_DW
			if (hw->dw_mmu_enable) {
				ret = av1_alloc_mmu_dw(hw, ibuf->mmu_box_dw,
					pic->index,
					pic->y_crop_width,
					pic->y_crop_height/2 + 64 + 8,
					hw->aom_param.p.bit_depth,
				...
				ret = av1_alloc_mmu_dw(hw, ibuf->mmu_box_dw_1,
					pic->index,
					pic->y_crop_width,
					pic->y_crop_height/2 + 64 + 8,
					hw->aom_param.p.bit_depth,
