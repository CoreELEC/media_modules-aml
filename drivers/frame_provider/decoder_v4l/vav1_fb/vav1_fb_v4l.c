				...
				struct PIC_BUFFER_CONFIG_s *pic_config)
{
	...
	if (vf) {
		...
		if (pic_config->double_write_mode &&
			...
			if ((get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_S4 || get_cpu_major_id() == AM_MESON_CPU_MAJOR_ID_S4D)
		...
		av1_print(hw, AV1_DEBUG_SEI_DETAIL,
			"%s aux_data_size:%d inst_cnt:%d vf:%px, pic %px, index %x, fb %px\n",
			__func__, pic_config->aux_data_size, vdec->inst_cnt, vf, pic_config, vf->index, fb);
...
#endif
			struct internal_comp_buf *ibuf = NULL;
			ibuf = index_to_icomp_buf(hw, cm->cur_frame->buf.index);
...
irqreturn_t vav1_back_threaded_irq_cb(struct vdec_s *vdec, int irq)
{
	...
	if (dec_status == HEVC_BE_DECODE_DATA_DONE) {
		...
		(debug & AOM_DEBUG_DIS_RECYCLE_MMU_TAIL) == 0) {
			...
			if (pic != NULL) {
				struct internal_comp_buf *ibuf = index_to_icomp_buf(hw, pic->index);
				/* is necessary ?
				hevc_mmu_dma_check(hw_to_vdec(hw));
				*/
				av1_print(hw, AOM_DEBUG_HW_MORE,
					"mmu free tail, index %d core0 0x%x, core1 0x%x\n",
					ibuf->index, used_4k_num0, used_4k_num1);
				decoder_mmu_box_free_idx_tail(ibuf->mmu_box,
					ibuf->index, used_4k_num0);

				decoder_mmu_box_free_idx_tail(ibuf->mmu_box_1,
					ibuf->index, used_4k_num1);
#ifdef AOM_AV1_MMU_DW
				if (hw->dw_mmu_enable) {
					...
					decoder_mmu_box_free_idx_tail(ibuf->mmu_box_dw,
						ibuf->index, used_4k_num0);

					decoder_mmu_box_free_idx_tail(ibuf->mmu_box_dw_1,
						ibuf->index, used_4k_num1);
					av1_print(hw, AOM_DEBUG_HW_MORE,
